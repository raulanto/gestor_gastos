...
class TransactionNotifier extends AsyncNotifier<List<TransactionEntity>> {
  ...
  Future<void> addTransaction(TransactionEntity transaction) async {
    ...
    state = await AsyncValue.guard(() async {
      ...
      ref.read(savingsServiceProvider).processTransactionRules(newT).catchError(
        (e) {
      ...
      for (final split in newT.splits) {
        watcher.checkBudgetForCategory(split.categoryId, txDate).catchError((
          e,
        ) {
    ...
    Provider<AsyncValue<List<TransactionEntity>>>((ref) {
      ...
      return transactionsState.whenData((transactions) {
        return transactions.where((t) {
          ...
          return tDate.year == selectedMonth.year &&
              tDate.month == selectedMonth.month;
        }).toList();
      });
    });

final transactionByIdProvider = Provider.family<TransactionEntity?, String>((
  ref,
  id,
) {
